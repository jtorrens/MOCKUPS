...
export type SaveState = "saved" | "dirty" | "invalid" | "saving" | "failed";

function stringifyJson(value: unknown): string {
  return JSON.stringify(value ?? {}, null, 2);
}

export function draftValue(record: AppRecord | undefined, field: AppFieldDefinition) {
  if (!record) return "";
...
interface RecordDraftAutosaveOptions {
...
export function useRecordDraftAutosave({
  table,
  record,
  onRecordsChanged,
  onRecordSaved,
}: RecordDraftAutosaveOptions) {
