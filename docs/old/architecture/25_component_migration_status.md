...
| `textInputBar` | system | Structurally migrated | Preview renders a screen-width bar with embedded `surface`, `textBox` and one `iconBar` slot. `idle` vs `typing` is derived from whether runtime text is empty. Text is explicitly forwarded Runtime data; placeholder, maximum lines and both Icon Row slots are Text Box Variant values customized only through local Overrides. |
