...
- `src/debug-ui/components/RecordEditor.tsx` remains the central coordinator for persistence, table dispatch, screen instance wiring, generic record fallback, and any remaining cross-entity glue.
...
- Continue extracting the remaining domain field handlers from `RecordEditor`, especially screen instance wiring, generic record fallback, and any remaining top-level editor composition glue.
