...
- `src/debug-ui/editors/ShotFields.tsx`, `RenderPresetFields.tsx`, `ProductionFields.tsx`, `ScreenInstanceFields.tsx`, `ActorFields.tsx`, `AppMediaFields.tsx`, and `ModuleBehaviorFields.tsx` own their table-specific field exceptions.
...
- `src/debug-ui/components/RecordEditor.tsx` remains the central coordinator for persistence, table dispatch, editor composition, token/group editing, Theme/App-specific token surfaces, and any remaining cross-entity glue.
...
- Continue extracting the remaining domain field handlers from `RecordEditor`, especially Theme chrome groups, App token root composition, Module Theme token groups, and generic table dispatch glue.
