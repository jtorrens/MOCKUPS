...
| `collectionStack` | atom | Structurally migrated | Generic ordered runtime collection for variable groups. Its single protected Default Variant has no composition; Flow/Stacked distribution, tokenized boundaries, stack direction/offset and all concrete child Variants/overrides/inputs are Runtime Inputs. Flow shares the Component Stack layout helper and permits fill/content sizing. Stacked always resolves fit-content, disables Sizing, uses collection order as z-order and offsets successive children down or up inside the same region. The atom owns no Notification rule or timer; stable item ids prepare the later explicit presence/enter/exit contract. |
| `notification` | component | Structurally migrated | One notification item composed from explicit Avatar and Label Variant slots. Variant data owns Avatar side and tokenized gap; runtime data owns Actor, text and subtext. Its resolver performs the composition and emits generic child primitives. |
| `notifications` | component | Structurally migrated | Runtime notification collection built on one embedded Collection Stack Variant. Its collection selector is restricted to Notification Variants; each item keeps stable id, overrides and child runtime inputs. Default distribution is Stacked/Fit content. Lifecycle and expanded state remain a later generic presence contract, not renderer state. |
| `label` | atom | Functional reference | Text/subtext, sizing, typography tokens, text align and generic subtext placement are on the new route. Text and Subtext independently expose runtime literal/count-up/count-down sources plus decimal size multipliers applied after token resolution. Calculated text uses the explicit owner-local frame and FPS and reaches the renderer as final per-frame text; invalid clock formats are errors. Visual surface is an embedded `surface` variant. |
