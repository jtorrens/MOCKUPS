...
| `bubble` | component | Structurally migrated | Embeds `surface`, `textBox`, optional actor `label`, and an optional selected media attachment slot. Variant data selects `none`, `image`, `video` or `audio` and stores one variant slot for each media kind; image/video resolve through `media`, audio resolves through `audio`. Runtime inputs define state (`incoming`, `system`, `outgoing`), max text width, text, actor name, write-on timing in frames, media source/viewport/playback and media actions. Variant data owns text padding and per-state light/dark palette color pairs. The parent will own message alignment. The bubble resolver owns state-to-tail mapping: incoming = left tail, system = no tail, outgoing = right tail; tail vertical/geometry stays in the selected surface variant. Text reveal currently uses the shared simple write-on frame helper and is prepared for richer future typing plans. |
