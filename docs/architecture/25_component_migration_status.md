...
The cross-component Stack/slot/State/Module Instance ownership contract is
maintained in
[31_structural_stacks_slots_and_module_instances.md](31_structural_stacks_slots_and_module_instances.md).
...
The experimental Simplified projection has been retired under contract 66.
The complete metadata-driven editor is the only authoring surface. Keypad and
Password retain their ordinary cards, embedded components, dictionary controls
and runtime behavior; no projection metadata or capture workflow remains.
...
| `textBox` | atom | Structurally migrated | Reusable text field atom. Size, text and placeholder are Runtime Inputs. Left and Right Icon Rows cross complete explicit child boundaries: full Icon Row Variant slot, structured stable Button items, local Overrides, independent tokenized gap/orientation and one tokenized gap to text. Variant owns surface, padding, typography, text colors, alignment, overflow mode and embedded cursor Variant. Flat icon lists and shared inferred row settings are retired. |
| `iconRow` | atom | Structurally migrated | Reusable row/column of stable `button` instances. Every item independently owns a full Button Variant reference, explicit local Overrides, runtime content mode, state, icon/text and Push action. The dictionary editor exposes explicit Component/Variant selection and Button Overrides without deriving identity from name, type or position. |
...
| `componentStack` | atom | Migrated | Generic ordered runtime collection of slots. Every slot owns an ordered nested state collection through the recursive generic Structured Collection dictionary control. State 1 is the default; later states expose an animatable Active boolean, Replace/Overlay behavior, explicit empty Component support, concrete Variant/Overrides/Inputs and generic Enter/Exit Motion. Every state owns generic Placement inside the frame assigned to its slot; slot order and fixed/reflow gap-before remain structural slot data. The shared collection footer always permits adding and reveals the new item; a new slot begins with one explicit empty default state. A forwarded collection projection exposes those slots at the next boundary as vertical runtime tabs. Test Values derives a target-State combo plus compact Play/Restore from the generic action contract; a persistent Module Instance instead exposes animatable `runtimeStateId` targeted by the stable slot id and hides source/transition/elapsed clocks. The projected Slots collection declares the generic `collectionFooter` animation presentation, so every direct `Slot N · State` track is aggregated below the complete slot list while State-owned child animations remain inside that State. Keyframe values are stable State ids. The resolver evaluates both v2 State-property tracks and preview actions per requested frame, applies deterministic collection-order selection, restarts child-local time on entry and keeps a frozen outgoing child resolved for its finite exit. The flat runtime item shape and the retired slot alignment are explicitly migrated without compatibility fallbacks. No Stack rule exists in the bridge, renderer or MainWindow. |
...
| `audio` | component | Functional reference, still evolving | Embeds `surface`, `avatar` and the generic `badge`. Badge visibility, Icon/Text content, size and palette colors are runtime inputs; the selected Badge Variant and its Audio-owned placement remain Variant configuration. Playback inputs are generic frame inputs exposed through a preview action. |
...
| `textInputBar` | system | Structurally migrated | Preview renders a screen-width bar with embedded `surface`, `textBox` and one `iconBar` slot. `idle` vs `typing` is derived from whether runtime text is empty. Text content is Runtime data passed into the embedded `textBox`; placeholder and the complete structured Left/Right Text Box Icon Row inputs are fixed by the parent Variant through generic input bindings and forwarded without reconstruction. |
