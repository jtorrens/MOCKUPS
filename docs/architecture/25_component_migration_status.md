...
| `textInputBar` | system | Structurally migrated | Preview renders a screen-width bar with embedded `surface`, `textBox` and left/right `iconRow` slots. The icon rows use generic variant input bindings for size/gap/orientation/button icon variant, while their icon-token lists remain runtime inputs. Text content is runtime data passed into the embedded `textBox`; placeholder is fixed by the parent variant through generic input bindings. |
