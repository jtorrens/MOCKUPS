...
Missing references stay visible. Media directories and Production Font family
directories show the directory path once and use `Media folder` or
`Font family folder` in the File name column instead of listing directory
contents.

Right-clicking the Absolute path or File name cell opens the same contextual
menu. It offers replacement of that exact media or font-family reference,
including a missing target, plus a separate `Show in Finder` action
...
and, when exposed, keyframes; a snapped playhead becomes amber. Item blocks use
a denser, higher-contrast diagonal hatch over the effective owner Motion/Fade
span at their leading edge and, only when they have an explicit outgoing
boundary, at their trailing edge. Its diagonal direction keeps it visually
distinct from the vertical boundary handles. The hatch does not change the
block's authored timing or editing behavior. Blocks can move horizontally and
can be shortened only from their outgoing edge. Their
...
Restore action appears immediately to the right; this remains true when that
value equals its inherited value. Restore clears the complete local boundary
