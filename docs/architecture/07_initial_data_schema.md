...
`config_json.tokens_json` stores app-level reusable defaults inherited by screens, such as generic app typography, wallpaper roles, icon references, shared surfaces, and mode-aware app colors. Wallpaper supports `kind: "solid" | "image"` and a shared decimal `opacity` in the `0–1` range. Mode colors live at `modes.light.wallpaper.color` and `modes.dark.wallpaper.color`; mode images live at `wallpaper.images.light.filePath` and `wallpaper.images.dark.filePath`. The active mode uses its image when it is declared and resolvable, otherwise it uses that same mode's wallpaper color. Actor owns the default wallpaper; an app inherits it by omitting `wallpaper` and may own the complete same contract when it requires an app-specific wallpaper. If an app wants to change a generic inherited role such as `colors.background` or `colors.accent`, it should override that same token path. Apps must not own visual status bar or navigation bar tokens; those resolve from Theme plus the final device/mode context, while per-page visibility remains module-instance behavior.
